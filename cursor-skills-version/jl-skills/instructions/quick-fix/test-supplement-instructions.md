...
1. **测试类结构**:
   ...
   - 测试方法命名：`testMethod1_正常流程()`、`testMethod1_异常流程()`
...
| 测试方法 | 场景描述 | 预期结果 |
|---------|---------|---------|
| `testMethod1_正常流程()` | 正常输入，验证成功 | 返回正确结果 |
| `testMethod1_边界值_空值()` | 输入为空，验证异常 | 抛出 IllegalArgumentException |
| `testMethod1_异常流程_业务异常()` | 业务规则不满足，验证异常 | 抛出 BusinessException |
| `testMethod2_正常流程()` | 正常输入，验证成功 | 返回正确结果 |
