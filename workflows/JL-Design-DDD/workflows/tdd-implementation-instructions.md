...
已加载所有设计文档和编码规范。我将作为您的**TDD 结对编程伙伴**，按照以下流程进行开发：

1. **Phase 1: 契约定义** (Client API, DTO, Domain Model)
2. **Phase 2: 编写测试 (Red)** (UnitTest, Mock)
3. **Phase 3: 最小化实现 (Green)** (ServiceImpl, Infrastructure)
4. **Phase 4: 规范检查 (Refactor)** (依赖倒置, MapStruct)
...
<action>基于 DDD 建模报告，生成核心代码骨架</action>

<action>生成 Client 层代码：
- `client/{_domain_}/api`: 接口定义
- `client/{_domain_}/dto`: Request/Response 对象
</action>

<action>生成 Domain 层核心代码：
...
<action>保存 Phase 1 代码到项目目录 (模拟)</action>

</step>

<step n="5.3" goal="Phase 2: 编写测试 (The 'Red' State)">

<action>针对 Application Service 或 Domain Service 编写单元测试</action>
...
请确认测试用例是否覆盖了关键业务场景？[确认通过 / 修改测试]</output>
...
<step n="5.5" goal="Phase 4: 规范检查与重构 (Compliance Check)">
...
<step n="5.6" goal="完成 TDD 阶段">

<action>将生成的代码保存到文件系统 (如果支持)</action>
<action>或者生成一份完整的代码清单 Markdown</action>

<output>**✓ TDD 代码实现完成**

我已为您生成了符合 Clean Architecture 的核心代码骨架。
